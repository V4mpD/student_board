...
const Announcements = () => {
  ...
  const fetchAnnouncements = useCallback(async () => {
    ...
    try {
      ...
      const response = await fetch(
        `${API_BASE_URL}/api/announcements?${userParams}`,
      );
      ...
      const formattedData = data.map((post) => ({
        ...
        priority: post.title.toLowerCase().includes("examen")
          ? "High"
          : "Medium",
      }));
  ...
  return (
    <div
      className="container page-padding position-relative"
        ...
        {posts.length === 0 ? (
          ...
          posts.map((post) => (
            <div key={post.id} className="col-12 mb-3">
              <div
                className={`card ${post.priority === "High" ? "border-danger" : ""}`}
              >
