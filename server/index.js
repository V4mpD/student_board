...
app.get('/api/chat', (req, res) => {
    ...
    const messages = db.prepare('SELECT * FROM chat_messages WHERE scope = ? AND target = ? ORDER BY created_at ASC LIMIT 100').all(scope, target);
    res.json(messages);
});

app.post('/api/chat', (req, res) => {
    const { sender_id, sender_name, content, scope, target } = req.body;
    const result = db.prepare('INSERT INTO chat_messages (sender_id, sender_name, content, scope, target) VALUES (?, ?, ?, ?, ?)').run(sender_id, sender_name, content, scope, target);
    res.json({ success: true, id: result.lastInsertRowid });
});
...
app.get('/api/deadlines', (req, res) => {
    ...
    const query = `
        ...
        WHERE target_group = ? 
        AND due_date >= datetime('now') -- Optional: Don't show old stuff
